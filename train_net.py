...
def train_epoch(
    ...
    data_size = len(train_loader)  ##959

    cur_global_batch_size = cfg.NUM_SHARDS * cfg.TRAIN.BATCH_SIZE###1
    num_iters = cfg.GLOBAL_BATCH_SIZE // cur_global_batch_size  ###64
    ...
    for cur_iter, (inputs, labels, _, meta) in enumerate(train_loader):
        ...
        lr = optim.get_epoch_lr(cur_epoch + float(cur_iter) / data_size, cfg)###0.005
        optim.set_lr(optimizer, lr)

#        train_meter.data_toc()
#        loss_fun=transformer_loss.VectorExtrema()
...
#          loss_fun = losses.get_loss_func(cfg.MODEL.LOSS_FUNC)(reduction="mean")
            loss_fun = torch.nn.CosineEmbeddingLoss(reduction="mean")
        ...
        if cfg.DETECTION.ENABLE:
            ...
            preds = model(inputs)  ####看到了这里

        # Compute the loss.
        #labels=One_Hot(labels.numpy(),5)
        preds=torch.nn.functional.softmax(preds)
        
        loss = loss_fun(preds, labels,torch.tensor([1]).cuda())
...
def train(cfg): ################train文件开始执行的入口
    ...
    # model.load_state_dict(state_stic,strict=False)
    ###########################
    if du.is_master_proc() and cfg.LOG_MODEL_INFO:
        misc.log_model_info(model, cfg, use_train_input=True)
